...
import { enhancedApiClient, JobStatusPoller } from '../services/enhanced-api';
...
type AppAction = 
  ...
  | { type: 'SET_TRANSLATION_METHOD'; payload: TranslationMethod }
  | { type: 'RESET_STATE' };
...
const initialState: AppState = {
  ...
  availableLanguages: [],
  isPolling: false,
  translationMethod: TranslationMethod.LLM
};
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'SET_TRANSLATION_METHOD':
      return { ...state, translationMethod: action.payload };
...
interface AppContextType {
  state: AppState;
  actions: {
    processVideo: (youtubeUrl: string, targetLanguage?: string, translationMethod?: TranslationMethod) => Promise<void>;
    cancelCurrentJob: () => Promise<void>;
    setSelectedLanguage: (language: string) => void;
    setTranslationMethod: (method: TranslationMethod) => void;
    clearError: () => void;
    resetState: () => void;
    loadLanguages: () => Promise<void>;
    startPolling: (jobId: string) => void;
    setIsProcessing: (isProcessing: boolean) => void;
    setCurrentJob: (job: ProcessingJob) => void;
  };
}

const EnhancedAppContext = createContext<AppContextType | undefined>(undefined);
...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  const loadLanguages = async (): Promise<void> => {
    try {
      const response = await enhancedApiClient.getSupportedLanguages();
  ...
  const processVideo = async (
    ...
    try {
      ...
      const request = {
        ...
        translationMethod: translationMethod || state.translationMethod
      };

      const response = await enhancedApiClient.processVideo(request);
  ...
  const cancelCurrentJob = async (): Promise<void> => {
    ...
    try {
      await enhancedApiClient.cancelJob(state.currentJob.id);
      pollerRef.current.stopPolling();
  ...
  const setTranslationMethod = (method: TranslationMethod): void => {
    dispatch({ type: 'SET_TRANSLATION_METHOD', payload: method });
  };
  ...
  const setIsProcessing = (isProcessing: boolean): void => {
    dispatch({ type: 'SET_PROCESSING', payload: isProcessing });
  };

  const setCurrentJob = (job: ProcessingJob): void => {
